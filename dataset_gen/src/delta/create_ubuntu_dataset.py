...
def get_random_utterances_from_corpus(candidate_dialog_paths,rng,utterances_num=9,min_turn=3,max_turn=20):
    ...
    for i in xrange(0,utterances_num):
        ...
        if(dialog_len<min_turn):
            ...
            #exit() -- ensured that dialog lengths meet minimum requirements
            continue
...
if __name__ == '__main__':
    ...
    def train_cmd(args):
        ...
        fixed_dialog_paths = []
        #Remove dialog_paths which have short conversations
        for dialog_path in dialog_paths:
            dialog =  translate_dialog_to_lists(dialog_path)

            dialog_len = len(dialog) - 1
            if(dialog_len>=3): #minimum_context_length
                fixed_dialog_paths.append(dialog_path)
                
                      
        train_set = create_examples(fixed_dialog_paths,
                                    args.examples,
