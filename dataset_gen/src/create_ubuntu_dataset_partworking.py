...
def get_random_utterances_from_corpus(candidate_dialog_paths,rng,utterances_num=9,min_turn=3,max_turn=20):
    ...
    print("Dialogs_num=%d" % dialogs_num)
    print("Utterances_num=%d" % utterances_num)
    ...
    for i in xrange(0,utterances_num):
        ...
        if(dialog_len<min_turn):
            '''
            print("--------------")
            print("Path =", dialog_path)
            print("Dialog len=", dialog_len)
            print("Dialog =", dialog)
            '''
            print("Dialog {} was shorter than the minimum required lenght {}. Skipping ...".format(dialog_path,dialog_len))
            #exit()
            continue
        ...
        print("turn_index = %d dialog[%d]=" % (turn_index, turn_index), dialog[turn_index])
...
def create_single_dialog_train_example(context_dialog_path, candidate_dialog_paths, rng, positive_probability,
    ...
    if positive_probability > rng.random():
        ...
        rand_utterance = get_random_utterances_from_corpus(candidate_dialog_paths,rng,1,
                                                     min_turn=minimum_context_length+1,
                                                     max_turn=max_context_length)
        print("rand_utterance=", rand_utterance)
        response = rand_utterance[0]
        label = 0.0
...
def convert_csv_with_dialog_paths(csv_file):
    ...
    paths = map(convert_line_to_path, csv_file)
    #print("paths =", paths)
    return paths
...
if __name__ == '__main__':
    ...
    def create_eval_dataset(args, file_list_csv):
        ...
        # training dataset
        #print("file_list_csv=", file_list_csv)
        ...
        '''
        print("f l csv=", file_list_csv)
        print("f=", convert_csv_with_dialog_paths(f))
        print("----------------")
        print("dialog_paths = ", dialog_paths)
        print("================")
        print("dialog_paths len = ", len(dialog_paths))
        '''
