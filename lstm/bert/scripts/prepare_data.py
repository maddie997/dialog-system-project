...
#Fom context and responses
#Remove __eot__
#Replace __eou__ with |||
def transform_sentence_to_bert(sentence):
  sent = sentence.replace('__eot__', ' ').replace('__eou__', '|||')
  lst=[]
  lst.append(sent)
  arr = bc.encode(lst)
  arr = arr[0]
...
def create_example_train(row, vocab):
  ...
  context_transformed = transform_sentence_to_bert(context)
  utterance_transformed = transform_sentence_to_bert(utterance)
  ...
  example.features.feature["context"].float_list.value.extend(context_transformed)
  example.features.feature["utterance"].float_list.value.extend(utterance_transformed)
...
def create_example_test(row, vocab):
  ...
  distractors = row[2:]
  '''
  print("vocab._tokenizer([context])=", (vocab._tokenizer([context])))
  print("vocab._tokenizer(next[context])=", next(vocab._tokenizer([context])))
  print("vocab._tokenizer([utterance])=", (vocab._tokenizer([utterance])))
  print("vocab._tokenizer(next[utterance])=", next(vocab._tokenizer([utterance])))
  '''
  context_len = len(next(vocab._tokenizer([context])))
  utterance_len = len(next(vocab._tokenizer([utterance])))
  #context_transformed = transform_sentence(context, vocab)
  ...
  #utterance_transformed = transform_sentence(utterance, vocab)
  utterance_transformed = transform_sentence_to_bert(utterance)
  #print("Context_transformed=", context_transformed)
  #print("Utterance_transformed=", utterance_transformed)
  ...
  for i, distractor in enumerate(distractors):
    ...
    dis_transformed = transform_sentence_to_bert(distractor)
    example.features.feature[dis_key].float_list.value.extend(dis_transformed)
  return example
...
def create_tfrecords_file(input_filename, output_filename, example_fn):
  ...
  for i, row in enumerate(create_csv_iter(input_filename)):
    ...
    if i==0:
      print("##############-----------------###############")
      print("create_tf_records_file:")
      print("row=", row)
      print("x=", x)
      print("##############-----------------###############")
      sys.error("Exit early")
    '''
