#Madhulima Pandey -- Added changes for handling of context/response encodings of size 768
...
def get_features(context, utterance):
  ...
  utterance_matrix = [utterance]
  context_len = len(context.split(" "))
  utterance_len = len(utterance.split(" "))
  # Madhulima: Context and utterance are floats.
  features = {
    "context": tf.convert_to_tensor(context_matrix, dtype=tf.float32),
    "context_len": tf.constant(context_len, shape=[1,1], dtype=tf.int64),
    "utterance": tf.convert_to_tensor(utterance_matrix, dtype=tf.float32),
